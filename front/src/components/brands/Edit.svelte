<script lang="ts">
    let { brandsModel } = $props();
    let id = $props.id();
</script>

{#if brandsModel.editDialog}
    <div
        class="w-full text-black h-full fixed top-0 left-0 flex flex-col items-center bg-transparent justify-center backdrop-blur-xl"
    >
        <div class="bg-white rounded-md p-4 w-96">
            <form onsubmit={(e) => brandsModel.editBrand(brandsModel.brand?.id, e)}>
                <h2 class="text-lg font-bold">Editar Marca</h2>
                <hr />

                <div class="p-2 flex flex-col">
                    <label for="name" class="block text-sm font-bold">Nombre de la marca:</label>
                    <input
                        class="border border-gray-400 rounded-md p-2"
                        placeholder="Ingrese el nombre de la marca"
                        type="text"
                        id={`name-${id}`}
                        name="name"
                        value={brandsModel.brand?.name}
                    />
                </div>

                <div class="p-2 flex justify-end gap-2 mt-3">
                    <button
                        type="button"
                        class="bg-red-400 text-white px-4 py-2 rounded-md"
                        onclick={() => (brandsModel.editDialog = false)}
                    >
                        Cancelar
                    </button>
                    <button
                        class="bg-blue-400 text-white px-4 py-2 rounded-md"
                        type="submit"
                    >
                        Guardar
                    </button>
                </div>
            </form>
        </div>
    </div>
{/if}
